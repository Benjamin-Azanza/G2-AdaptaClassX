...
export function createApp(root: HTMLElement): void {
  ...
  debugControls.innerHTML = `
    ...
    <div class="panel-group">
      <p class="panel-group__title">Pirate</p>
      <label class="toggle-row"><input id="pirate-bounds"     type="checkbox" /> Visual bounds</label>
      <label class="toggle-row"><input id="pirate-hitbox"     type="checkbox" /> Hitbox</label>
      <label class="toggle-row"><input id="pirate-attack-box" type="checkbox" /> Attack box</label>
    </div>

    <div class="panel-group">
      <p class="panel-group__title">Skeleton</p>
      <label class="toggle-row"><input id="skeleton-bounds"     type="checkbox" /> Visual bounds</label>
      <label class="toggle-row"><input id="skeleton-hitbox"     type="checkbox" /> Hitbox</label>
      <label class="toggle-row"><input id="skeleton-attack-box" type="checkbox" /> Attack box</label>
    </div>
  ...
  const pirateBounds     = debugControls.querySelector<HTMLInputElement>('#pirate-bounds');
  const pirateHitbox     = debugControls.querySelector<HTMLInputElement>('#pirate-hitbox');
  const pirateAttackBox  = debugControls.querySelector<HTMLInputElement>('#pirate-attack-box');
  const skelBounds       = debugControls.querySelector<HTMLInputElement>('#skeleton-bounds');
  const skelHitbox       = debugControls.querySelector<HTMLInputElement>('#skeleton-hitbox');
  const skelAttackBox    = debugControls.querySelector<HTMLInputElement>('#skeleton-attack-box');
  ...
  if (
    !pauseToggle || !showWorld ||
    !pirateBounds || !pirateHitbox || !pirateAttackBox ||
    !skelBounds   || !skelHitbox   || !skelAttackBox   ||
    !sceneReadout || !pointerReadout || !inputReadout
  ) {
  ...
  pirateBounds.addEventListener('change', () => {
    const s = debugStore.getState();
    debugStore.patchState({ pirate: { ...s.pirate, bounds: pirateBounds.checked } });
  });
  pirateHitbox.addEventListener('change', () => {
    const s = debugStore.getState();
    debugStore.patchState({ pirate: { ...s.pirate, hitbox: pirateHitbox.checked } });
  });
  pirateAttackBox.addEventListener('change', () => {
    const s = debugStore.getState();
    debugStore.patchState({ pirate: { ...s.pirate, attackBox: pirateAttackBox.checked } });
  });

  skelBounds.addEventListener('change', () => {
    const s = debugStore.getState();
    debugStore.patchState({ skeleton: { ...s.skeleton, bounds: skelBounds.checked } });
  });
  skelHitbox.addEventListener('change', () => {
    const s = debugStore.getState();
    debugStore.patchState({ skeleton: { ...s.skeleton, hitbox: skelHitbox.checked } });
  });
  skelAttackBox.addEventListener('change', () => {
    const s = debugStore.getState();
    debugStore.patchState({ skeleton: { ...s.skeleton, attackBox: skelAttackBox.checked } });
  });
  ...
  debugStore.subscribe((state) => {
    ...
    pirateBounds.checked    = state.pirate.bounds;
    pirateHitbox.checked    = state.pirate.hitbox;
    pirateAttackBox.checked = state.pirate.attackBox;
    skelBounds.checked      = state.skeleton.bounds;
    skelHitbox.checked      = state.skeleton.hitbox;
    skelAttackBox.checked   = state.skeleton.attackBox;
