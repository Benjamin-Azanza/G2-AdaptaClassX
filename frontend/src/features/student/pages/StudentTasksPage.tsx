  ...
  const loadAssignments = useCallback(() => {
    ...
    studentAssignmentsService
      ...
      .catch((requestError: unknown) => {
        setPending([]);
        setCompleted([]);
        setError(getApiErrorMessage(requestError, 'No pudimos cargar tus tareas.'));
      })
                    ...
                    >
                      <div className="flex flex-col gap-sm md:flex-row md:items-start md:justify-between">
                        <div className="flex-1">
                          ...
                          <h4 className="mt-sm font-headline text-lg font-bold md:text-xl">
                            {task.gameTitle}
                          </h4>
                          <p className="text-sm text-on-surface-variant">
                            Tiempo requerido: <strong>{task.minutosRequeridos} min</strong> ·
                            Vence el {formatDate(task.fechaLimite)}
                          </p>
                        </div>
                        <Link
                          to={task.gameRoute}
                    ...
                    <div className="flex items-center gap-sm">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-on-surface-variant">
                          Completado el{' '}
                          {task.completadoAt ? formatDate(task.completadoAt) : '-'}
                        </p>
                      </div>
                    </div>
