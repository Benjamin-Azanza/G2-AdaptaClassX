...
import { MOCK_STUDENT_PROFILE } from '../services/student.service';
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-background text-on-surface">
      ...
      <aside className="fixed left-0 top-0 z-50 hidden h-full w-64 flex-col border-r-4 border-on-background bg-surface-container-low p-md shadow-[4px_0_0_0_#1d1c17] md:flex">
        <div className="mb-lg">
          <Link to={routePaths.studentDashboard} className="font-headline text-3xl font-bold uppercase tracking-normal text-primary">
            Adapta Class
          </Link>
          <p className="mt-xs text-sm font-medium uppercase text-on-surface-variant">Zona Estudiante</p>
        </div>
        ...
        <div className="mt-auto space-y-sm">
          ...
          <div className="border-2 border-on-background bg-surface-container-high p-sm">
            <div className="mb-xs flex items-center justify-between">
              <span className="font-mono text-xs font-bold uppercase text-on-surface-variant">Nivel {profile.nivel}</span>
            ...
            <div className="h-3 w-full overflow-hidden border-2 border-on-background bg-surface-variant">
              <div className="h-full bg-primary transition-all" style={{ width: `${(profile.xp % 1000) / 10}%` }} />
            </div>
          </div>
          {/* User card */}
          <div className="flex items-center gap-sm border-2 border-on-background bg-surface-container-high p-sm">
            ...
            <div className="min-w-0 flex-1 overflow-hidden">
              ...
              <div className="flex items-center gap-xs">
                ...
                <span className="font-mono text-xs font-bold text-orange-500">{profile.racha} días</span>
              </div>
            </div>
            <button className="material-symbols-outlined text-on-surface-variant hover:text-error" onClick={handleLogout} title="Cerrar sesión" type="button">
              logout
            </button>
          </div>
        </div>
